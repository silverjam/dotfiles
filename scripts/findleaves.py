...
cache_path = os.path.expanduser("~/.find_leaves_cache.json")

if not os.path.exists(cache_path):
    cache = {}
else:
    cache = json.load(file(cache_path))

def perform_find(mindepth, prune, dirlist, filename):
  k = repr([mindepth, prune, dirlist, filename])
  sys.stderr.write("findleaves: key = " + k + "\n")
  if k in cache:
    sys.stderr.write("findleaves: cached\n")
    return cache[k]
  result = []
...
if __name__ == "__main__":
  main(sys.argv)
  json.dump(cache, file(cache_path, 'w'))
